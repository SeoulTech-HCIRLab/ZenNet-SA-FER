...
from . import posterv2
from ModelLoader.posterv2.models.ir50 import load_pretrained_weights
...
def _get_model_(arch, num_classes, pretrained=False, opt=None, argv=None):
    ...
    # add poster to teacher model list @by JinniPi
    elif arch == 'poster':
        model = posterv2._get_poster(num_classes=num_classes, pretrain=pretrained)
    elif arch == 'ir50':
        model = myresnet.MyIR50(num_classes=num_classes, head='linear')
    elif arch == 'ir50_sup':
        model = myresnet.MyIR50Sup(num_classes=num_classes, head='linear', path=opt.load_parameters_from)
    else:
