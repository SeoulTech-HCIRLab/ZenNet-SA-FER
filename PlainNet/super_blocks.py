...
class PlainNetSuperBlockClass(basic_blocks.PlainNetBasicBlockClass):
    ...
    def get_info_superblocks_test(self):
        for idx, block in enumerate(self.block_list):
            print("idx = {}, block type {}".format(idx, type(block)))
    ...
    # custom code by JinniPi get number block
...
class SuperConvKXBNRELU(PlainNetSuperBlockClass):
    ...
    ## code get deep_mad score, custom code by JinniPi

    def get_deepmad_forward(self):
        #init array
        output = np.zeros(self.sub_layers)
        for index, block in enumerate(self.block_list):
            output[self.index_pos[index]] += block.get_deepmad_forward()[0]
        return output
    ...
    def get_width(self):
        output = np.ones(self.sub_layers)
        # tmp_list = []
        for index, block in enumerate(self.block_list):
            # tmp_list.append(block.get_width()[0])
            output[self.index_pos[index]] *= block.get_width()[0]
        # print(tmp_list)
        return output
...
if __name__ == '__main__':
    ...
    model = SuperConvK1BNRELU(in_channels=32, out_channels=64, stride=2, sub_layers=2)
    print(model.index_pos)
    print(model.block_list)
    # print(model.get_deepmad_forward())
    print(model.get_width())