...
class SuperResIDWEXKX(PlainNetSuperBlockClass):
    def __init__(self, in_channels=None, out_channels=None, stride=None, bottleneck_channels=None, sub_layers=None,
        ...
        for j in range(self.sub_layers):
            ...
            if self.use_se:
                ...
                if dw_channels >= 32 and ((dw_channels / 8) // 2 == 0):
                    groups = 8
                else:
                    groups = 4
                inner_str += 'SA({},{})'.format(dw_channels, groups)
                index_pos_tmp1 += [i]
   ...
   ## custom get deepmad score by JinniPi
    def get_deepmad_forward(self):
        ...
        for index, block in enumerate(self.block_list):
            # print(index, block)
            # print(block.get_deepmad_forward())
            output[self.index_pos[index] - 1] += block.get_deepmad_forward()[0]
        # print("output of deepmad: ", output)
        return output
...
if __name__ =="__main__":
    ...
    model = SuperResIDWE2K7(in_channels=96, out_channels=224, stride=2, bottleneck_channels=224, sub_layers=5, use_sa=True, no_reslink=False)
    # out = model.get_deepmad_forward()
    # print(out)
    # width = model.get_width()
    # print(width)
    # print(out)
    # print(model.index_pos)
    # print(len(model.index_pos))
