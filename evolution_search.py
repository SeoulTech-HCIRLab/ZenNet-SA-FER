...
from ZeroShotProxy.compute_deepmad import ComputeDeepMadScore
...
def parse_cmd_options(argv):
    ...
    ## custom by JinniPi
    parser.add_argument('--efficient_score', type=float, default=0.5, help='score of depth')
    parser.add_argument('--check_score', action='store_true')
    parser.add_argument('--use_depth_penalty', action='store_true', help='penalty when the network is too deep')
    parser.add_argument('--use_deepmad_score', action='store_true', help='use deepmad score')
    parser.add_argument("--weight_zs", type=float, default=1.0, help='ratio of zenscore')
    parser.add_argument("--weight_dms", type=float, default=1.0, help='ratio of deepmad')
...
def main(args, argv):
    ...
    for loop_count in range(args.evolution_max_iter):
        ...
        #custom add efficent score by JinniPi
        if args.efficient_score is not None:
            the_model = AnyPlainNet(num_classes=args.num_classes, plainnet_struct=random_structure_str,
                                        no_create=False, no_reslink=True)
            # print(the_model)
            efficient_score = the_model.get_efficient_score(resolution=args.input_image_size)
            # print(efficient_score)
            if efficient_score > args.efficient_score:
                continue

        the_nas_core = compute_nas_score(AnyPlainNet, random_structure_str, gpu, args)
        # print(the_nas_core)
        the_nas_org = the_nas_core

        if args.use_depth_penalty and not args.use_deepmad_score:
            depth_penalty = the_model.get_depth_penalty(depth_penalty_ratio=5)
            the_nas_core = the_nas_core - depth_penalty

        if args.use_deepmad_score:
            the_compute_deepmad =  ComputeDeepMadScore(image_size=args.input_image_size, multi_block_ratio = [1,1,1,1,1],
                                                       ratio = 1, init_std = 1, init_std_act = 1, logger=None,
                                              kwargs={'alpha1': 1, 'alpha2': 1})
            score_deepmad = the_compute_deepmad(the_model)
            # print(score_deepmad)
            #print("log", np.log(score_deepmad))
            #print(the_nas_core)
            the_nas_core = args.weight_zs*the_nas_core + args.weight_dms*np.log(score_deepmad)
        ...
        if args.check_score:
            max_score = max(popu_zero_shot_score_list)
            min_score = min(popu_zero_shot_score_list)
            with open(file='zenscore_check.txt', mode='a+') as f:
                line = str(loop_count) + '\t' + str(the_nas_org) + '\t' + str(score_deepmad) + '\t' + str(
                    np.log(score_deepmad)) + '\t' + str(max_score) + '\t' + str(min_score) + '\n'
                f.write(line)
