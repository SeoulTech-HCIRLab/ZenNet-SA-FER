...
import global_utils
from ModelLoader.posterv2 import RecorderMeter1 , RecorderMeter
...
class TeacherStudentDistillNet(nn.Module):
    def __init__(self, teacher_model: nn.Module, student_model: nn.Module, opt, argv, target_downsample_ratio=None):
        ...
        # print("teacher_stage_feature_list", len(teacher_stage_features_list))
        # print("student_stage_features_list", len(student_stage_features_list))
        # breakpoint()

        if len(teacher_stage_features_list) != len(student_stage_features_list):
    ...
    def forward(self, x):
        if self.training:
            ...
            # print("teacher stage feature list", self.teacher_stage_features_list)
            # print("teacher stage feature list", self.teacher_stage_features_list)
    ...
    def compute_ts_distill_loss(self):
        ...
        #logit_loss = self.kldiv(F.log_softmax(self.student_logit / self.temperature, dim=1),
        #                               F.softmax(self.teacher_logit / self.temperature, dim=1)) * (self.temperature ** 2)
        # print("logit_loss", logit_loss)
        # print("temperature", soft_target_loss)
